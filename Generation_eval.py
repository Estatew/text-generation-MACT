...
def make_refer(test):
    refer_dict = defaultdict(dict)
    for text, movie_id, score in test:
        if score not in refer_dict[movie_id]:
            refer_dict[movie_id][score] = [text]
        else:
            refer_dict[movie_id][score].append(text)
    return refer_dict
...
def main(_):
    with tf.Graph().as_default(), tf.Session(config=config_tf) as session:
        ...
        print('epoch_%d' % config.eval_eopch)
        #  + 'epoch_%d' % config.eval_eopch
        model_saver.restore(session, config.model_path + "epoch_10")
        print('Done!')

        # test_topic = [2, 27 + 3]
        test = load_pikle(config.test_pickle)
        refer_dict = make_refer(test)
        gen_dict = defaultdict(dict)
        generate_file = config.generate_file
        f_w = open(generate_file, "w", encoding="utf-8")
        for a1 in tqdm(range(config.movie), desc="attr1"):
            for a2 in tqdm(range(1, config.score + 1), desc="attr2"):

                test_topic = [a1, config.movie - 1 + a2]
                ...
                if config.is_sample:
                    try:
                        ...
                        top_indices = np.random.choice(a=config.vocab_size, size=beam_size, replace=False,
                                                       p=prob.reshape(-1))
                    except:
                        top_indices = np.random.choice(a=config.vocab_size, size=beam_size, replace=True,
                                                       p=prob.reshape(-1))
                else:
                    ...
                    for b in beams:
                        ...
                        if config.is_sample:
                            try:
                                ...
                                top_indices = np.random.choice(config.vocab_size, beam_size, replace=True,
                                                               p=prob.reshape(-1))
                        else:
                            top_indices = np.argsort(-y1)
                        for i in range(beam_size):
...
#                 remove_set = {'[PAD]', '[MASK]', '[CLS]', '[UNK]'}
                remove_set = {'[PAD]', '[MASK]', '[UNK]'}
                for w in res_text:
                    if w in remove_set:
                        continue
                    final_text.append(w)
                generated = ''.join(final_text)
                info = "\n{} epoch {}; a1 {}; a2 {}; sample {} {}\n".format(
                    "=" * 5, config.eval_eopch, a1, a2, str(generated), "=" * 5)
                print(info)
                f_w.write(info)
                if a2 not in gen_dict[a1]:
                    gen_dict[a1][a2] = [final_text]
                else:
                    gen_dict[a1][a2].append(final_text)
        f_w.close()
#         """
#         算这一个epoch模型的bleu
#         """
#         # reference_corpus = [["今天真开心", "今天真快乐啊"], ["前天真开心", "今天真快乐"]]
#         # translation_corpus = ["今天真快乐", "昨天真开心"]
#         reference_corpus = []
#         translation_corpus = []
#         for movie_id, mvalue in tqdm(gen_dict.items(), desc="gen_dict"):
#             for score, svalue in tqdm(mvalue.items(), desc="gen_dict"):
#                 for x in svalue:
#                     try:
#                         reference = refer_dict[movie_id][score]
#                         reference_corpus.append(reference)
#                         translation_corpus.append(x)
#                     except:
#                         print("error: movie_id {}  score {}".format(movie_id, score))
#                         continue
#         eval_file = config.eval_file
#         with open(eval_file, "w", encoding="utf-8") as res_f:
#             for bleu in tqdm(range(1, 5), desc="cal bleu"):
#                 res = compute_bleu(reference_corpus, translation_corpus, max_order=bleu, smooth=False)
#                 res = str(res)
#                 res_f.write("epoch:{}  bleu:{} res:{} \n".format(config.eval_eopch, bleu, res))

#             print("cal rough")
#             res = cal_rough(reference_corpus, translation_corpus)
#             res = str(res)
#             res_f.write("\n\n epoch:{} res:{} \n".format(config.eval_eopch,  res))
